...
データを一次元状に配列したものをListと呼ぶことが多く、JavaではArray
もLinkedListもListの一種という分類です。その中でも以下のような特徴を
備えたものはStack(スタック)やQueue(キュー)などと呼ばれ、先頭や末尾への
操作が可能なものは中身はLinkedListのような数珠つなぎの構造として
実装されています。

- Stack
  - 末尾にへの追加 push()と取り出し pop()だけできる。
  - 再帰処理をオーバーフローしないようにループに書き換える場合などに便利。
- Queue
...
- Vector, Array
  ...
  - メモリの利用効率が良く、ランダムアクセスも速いのでよく使われる。 
- Set
  - 重複のないデータの集合
...
- 環状リスト
  ...
  - 季節や時間など、周期性のあるデータを扱う場合に原点で区切ることなくにシームレスに扱うことができます。
  
- HashSet, HashTable, HashMap (Java, C++、Python(dict)、Perl(ハッシュ)等)
  - 重複のないデータを作りたい場合や、データ同士の対応付けに便利なコレクションです。
