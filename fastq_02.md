...
```
例1はC/C++は問題ないメモリの取得方法ですが、Pythonや、Javaなどの他の言語では見られない方法です。多くの言語では例2のような方式になっています。特にPythonは関数内の局所的な変数でも処理系内部ではこのようにヒープからメモリを取得して使用する仕組みになっており、実質的にスタックのサイズに制限がないとされてるので、スタックの容量を意識する必要がないことから、それも人気の一つの要素になっているのかもしれません。
