...
物理的にはどちらも同じ物理的なデバイス上に保持されるので、読み書きの速度には違いはありません。スタックは主に自動変数に使われます。つまり一時的な記憶のためのメモリとして使われます。一方、ヒープは長時間や広いスコープに渡って記憶するための領域として利用されます。スタックとヒープで大きな違いがあるのはメモリの確保に要する時間的、計算量的コストでです。スタックメモリは関数から出てもその領域は解放されても他の処理に使われるわけではなく、次の関数呼び出しがあればすぐに再利用されます。それというのも、スタックがプッシュされる、ポップされるという操作は実際には計算機の内部ではスタックポインタを上下させるだけで実現しており、積み下ろしのようなデータの移動はなく、ポインタの書き換えだけで積み下ろしに相当することをやっています。またスタックは上限が8MBなので、確保しっぱなしにしてもそれほど邪魔にはなりません。一方、ヒープメモリからのメモリの確保は、指定したメモリ容量ぶんだけの連続した空き領域を探してこなければならないので、もしなければ他のプロセスが使っているメモリを移動したり、場合によってはページアウトでHDDに一時的に書き出したりしなければならないこともあるので、ひどい場合にはコストは100倍では済みません。
